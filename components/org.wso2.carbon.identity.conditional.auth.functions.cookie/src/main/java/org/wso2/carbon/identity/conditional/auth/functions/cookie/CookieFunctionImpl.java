...
import org.wso2.carbon.identity.conditional.auth.functions.cookie.util.Constants;
...
public class CookieFunctionImpl implements SetCookieFunction, GetCookieFunction {
    ...
    public void setCookie(JsServletResponse response, String name, String value, Map<String, Object> properties) {
        ...
        boolean sign = Optional.ofNullable((Boolean) properties.get(Constants.SIGN)).orElse(false);
        boolean encrypt = Optional.ofNullable((Boolean) properties.get(Constants.ENCRYPT)).orElse(false);
        ...
        try {
            cookieValueJson.put(Constants.VALUE, value);
            cookieValueJson.put(Constants.SIGNATURE, signature);
    ...
    public String getCookieValue(JsServletRequest request, String name, Map<String, Object> properties) {
        ...
        for (Cookie cookie : cookies) {
            if (name.equals(cookie.getName())) {
                boolean validateSignature = Optional.ofNullable((Boolean) properties.get(Constants.VALIDATE_SIGN))
                        .orElse(false);
                boolean decrypt = Optional.ofNullable((Boolean) properties.get(Constants.DECRYPT))
                        .orElse(false);
                JSONObject cookieValueJSON;
                try {
                    cookieValueJSON = new JSONObject(new String(Base64.decode(cookie.getValue()),
                            Charsets.UTF_8));
                } catch (JSONException e) {
                    log.error("Error occurred when converting cookie value to JSON.", e);
                    return null;
                }
                String valueString = cookieValueJSON.getString(Constants.VALUE);
                if (decrypt) {
                    try {
                ...
                if (validateSignature) {
                    byte[] signature = Base64.decode((String) cookieValueJSON.get(Constants.SIGNATURE));
                    try {
